DELIMITER //
#DROP PROCEDURE IF EXISTS Insert_New_Cow
CREATE PROCEDURE Insert_New_Cow(input_herd_id varchar(16),input_cow_id varchar(20),input_calf_id_at_birth varchar(2),input_birth_date date,input_breed VARCHAR(16),
			...
			input_date_entered_herd date ,input_source boolean,input_active_flag varchar(3),input_animal_source ENUM('S','P'),input_date_culled date,input_reason_culled varchar(2),input_cull_comments VARCHAR(100),input_profile_notes text)		
BEGIN
#------------IF THIS ANIMAL DOESN'T EXIST IN THE SYSTEM THEN ADD A NEW ENTRY IN CATTLE_INFO_TBL--------------#
	IF NOT EXISTS(select 1 from cattle_info_tbl where animal_id=input_calf_id_at_birth) THEN
		...
		INSERT into cattle_info_tbl(herd_id,cow_id,birth_date,breed,reg_no,reg_name,elec_id,sire_id,dam_id) 
		values(input_herd_id,input_birth_date,input_breed,input_reg_num,input_reg_name,input_elec_id,input_sire_id,input_dam_id);
		...
		INSERT into notes_tbl(chaps_id,entry_date,note_type,notes) values(LAST_INSERT_ID(),CURDATE(),'p',profile_notes);
		#check if the user enters the cull information, if yes, enter it into the cull_tbl
		IF NOT ISNULL(input_date_culled) THEN
			INSERT into cull_tbl(chaps_id,cow_id,cull_date,cull_code,cull_comments) values(@my_chaps_id,input_cow_id,input_date_culled,input_cow_idinput_reason_culled,input_cull_comments);
		END IF;
	ELSE 
		IF NOT ISNULL(input_calf_id_at_birth) THEN
			...
			insert into cattle_names_tbl(chaps_id,entry_date,cattle_name,entry_type) values(@my_chaps_id,CURDATE(),cow_id,'CO');
			...
			END IF;
		END IF;
			#else if the animal already exists as a claf then enter one more record in the cattle_name_tvbl for its updated name as a cow
	END IF;
END//
