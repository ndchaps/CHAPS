...
BEGIN 
	Select * from(
			select  wt.calf_id, wt.entry_date as date_weighed,wt.manage_code,wt.contemp_grp,wt.muscle_grade,mt.lot_no,mt.weight,mt.hip_height,mt.frame_score,mt.`status`,
			...
			left join notes_tbl nt on wt.chaps_id=nt.chaps_id where mt.entry_type='W'
	)A;
END//
