...
DELIMITER //
#DROP PROCEDURE Insert_CalfWeanInfo_byPK//
CREATE PROCEDURE `Insert_CalfWeanInfo_byPK`(input_chaps_id int,input_date_weighed date,input_wean_weight float,input_manage_code varchar(2),input_wean_status varchar(2),input_hip_height float(30),input_date_measured date,input_contemp_grp varchar(2),input_muscle_grade varchar(2),input_frame_score float,input_lot varchar(2),entry_type varchar(2),input_weaning_notes text)
BEGIN
CASE WHEN  NOT EXISTS(select 1 from weaning_tbl where chaps_id=input_chaps_id) THEN 
	INSERT into weaning_tbl(input_chaps_id,entry_date,manage_code,contemp_grp,muscle_grade,lot_no) values(input_chaps_id,input_date_weighed,input_manage_code,input_contemp_grp,input_muscle_grade,input_lot_no);
	INSERT into measurement_tbl(input_chaps_id,entry_date,weight,hip_height,frame_score,`status`,entry_type) values(input_chaps_id,input_date_weighed,input_wean_weight,input_hip_height,input_frame_score,input_wean_status,'w');
...
ELSE
	...
	where chaps_id=input_chaps_id;
	UPDATE notes_tbl 
			SET CASE WHEN NOT ISNULL(input_weaning_notes) THEN notes= input_weaning_notes,entry_date=CURDATE() end
	where chaps_id=input_chaps_id;
END CASE;
END//
